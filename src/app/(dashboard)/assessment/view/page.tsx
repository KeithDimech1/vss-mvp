  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="mb-6">
        <Link
          href="/dashboard"
          className="inline-flex items-center text-[#0D8BFF] hover:text-[#2C3E7C] font-medium mb-4 transition-colors"
        >
        ...
        <h1 className="text-4xl font-bold text-[#2C3E7C]">Your Assessment Responses</h1>
      ...
      {/* User Info Card */}
      <div className="bg-gradient-to-r from-[#0D8BFF] to-[#2C3E7C] rounded-xl shadow-xl p-8 mb-8 text-white">
        <div className="flex items-start justify-between">
          <div className="flex items-center">
            {session && (
              <>
                <div className="w-16 h-16 bg-white rounded-full flex items-center justify-center text-[#2C3E7C] font-bold text-2xl mr-6 shadow-lg">
                  {session.fullName.charAt(0)}
                </div>
                <div>
                  <h2 className="text-2xl font-bold mb-1">{session.fullName}</h2>
                  <p className="text-blue-100">@{session.username}</p>
                </div>
              </>
            )}
          </div>
          <div className="text-right">
            <p className="text-blue-100 text-sm mb-1">Submitted on</p>
          ...
          return (
            <div key={question.id} className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
              ...
              <div className="flex items-start mb-4">
                <div className="flex-shrink-0 w-10 h-10 bg-gradient-to-br from-[#0D8BFF] to-[#2C3E7C] rounded-lg flex items-center justify-center text-white font-bold mr-4">
                  {index + 1}
                </div>
                <div className="flex-grow">
                  <h3 className="text-lg font-semibold text-[#2C3E7C] mb-2">
                    {question.question}
                  </h3>
                  {question.helpText && (
                  ...
                  <div className="mt-3 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-100">
                    {question.type === 'radio' && question.options ? (
                      <div className="flex items-center">
                        <svg className="w-5 h-5 text-[#0D8BFF] mr-2" fill="currentColor" viewBox="0 0 20 20">
      ...
      <div className="mt-8 bg-gradient-to-r from-green-50 to-emerald-50 rounded-xl p-6 border border-green-100">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <p className="text-2xl font-bold text-[#2C3E7C]">{system1Questions.length}</p>
          </div>
          <div className="bg-white rounded-lg p-4 shadow-sm">
      ...
      <div className="mt-8 flex gap-4">
        <Link
          href="/dashboard"
          className="px-6 py-3 bg-[#0D8BFF] text-white rounded-lg hover:bg-[#2C3E7C] transition-colors font-medium shadow-md hover:shadow-lg"
        >
          ← Back to Dashboard
        </Link>
        <button
          onClick={() => window.print()}
          className="px-6 py-3 bg-white text-[#2C3E7C] rounded-lg hover:shadow-md transition-all border-2 border-[#0D8BFF] font-medium"
        >
