# FORM 9.4 - PROFESSIONAL STAFF EXPERIENCE
## GDAC-SA RFQ Pre-Qualification Response

**Applicant:** Lithodat Pty Ltd
**ABN:** 63 627 008 904
**Date:** November 2025
**Version:** 1.0 (Submission Ready)

---

## PROFESSIONAL/TECHNICAL STAFF EXPERIENCE TABLE

| # | Name | Function | Specialization / Area of Expertise | Years of Experience |
|---|------|----------|-----------------------------------|---------------------|
| 1 | **Dr. Behnam Sadeghi** | Machine Learning Technical Advisor | Machine Learning, Geospatial AI, Python Development (PyImpetus, PyMiner), Mathematical Geosciences, Critical Minerals | 15+ years |
| 2 | **Pedro Nogueira Ferreira** | AI Software Development Lead | AI/ML Strategy & Implementation, Enterprise Cloud Architecture (Azure/AWS), Digital Transformation, Backend Migrations | 25+ years |
| 3 | **Dr. Qusay Abeed** | Senior Geological Consultant | Petroleum Geochemistry, Basin Modelling, CO2 Storage (CCUS), Petroleum Systems Analysis, Arabic Speaker | 17+ years |
| 4 | **Vinko Novak** | Head of Data Security | Data Security, Information Protection, Compliance, Security Architecture | 10+ years |
| 5 | **Xinyan Zhang** | Frontend Developer | React, TypeScript, UI/UX implementation, Responsive design | 3+ years |
| 6 | **Lujia Yang** | Frontend Developer | React, TypeScript, UI/UX implementation, Responsive design | 3+ years |
| 7 | **Tarun Sengar** | Backend Developer | Java, AWS, Backend Systems, API Development | 5+ years |
| 8 | **Nirali Dudharejiya** | Backend Developer | Java, Liquibase, Backend Systems, Database Management | 5+ years |
| 9 | **Cris Ibarra** | Data Quality Specialist | Data Entry, Data Validation, Quality Control, Database Operations | 3+ years |
| 10 | **Perla Luque** | Data Quality Specialist | Data Entry, Data Validation, Quality Control, Database Operations | 3+ years |
| 11 | **Dr. Alejandra Bedoya** | Data Entry Specialist / PhD Candidate | Geochronology, Thermochronology, Tectonics, Data Validation, Geological Engineering | 10+ years |
| 12 | **Dr. Fun Meeuws** | Geochemistry Data Specialist | Fission Track Thermochronology, Thermal History Modelling, Geochemistry, Seismic Interpretation, GPlates | 15+ years |
| 13 | **Benjamin Dib** | Data Clerk (LithoSpace) | Space Science, Geospatial Data, Moon/Mars Mission Data, R Programming, QA Testing | 3+ years |

---

## DETAILED TECHNICAL PROFILES

### Technical Leadership (also listed in Form 9.3 for governance)

---

#### Dr. Fabian Kohlmann - Chief Geoscientist / Technical Lead

| Field | Details |
|-------|---------|
| **Position** | Managing Director & Chief Geoscientist |
| **Years with Lithodat** | 7 years (Founder, Dec 2017) |
| **Total Experience** | 15+ years |
| **Education** | PhD Geosciences, University of Melbourne (2012) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **Geoscience** | Thermochronology, (U-Th)/He, fission track, thermal history modeling |
| **Data Systems** | FAIR geochemistry frameworks, relational database architecture, SKOS vocabularies |
| **Platform Design** | EarthBank architecture, Isotopes.au ontologies, multi-source data harmonization |
| **Industry Tools** | Halliburton wireline logging systems, Landmark software |

---

#### Dr. Wayne Noble - CIO / Technology Lead

| Field | Details |
|-------|---------|
| **Position** | Chief Information Officer / Executive Director |
| **Years with Lithodat** | 6+ years (Since Dec 2018) |
| **Total Experience** | 25+ years |
| **Education** | PhD Geology (La Trobe, 1998); Grad Dip CS with Distinction (RMIT, 2006); BSc Honours (La Trobe, 1992) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **Software Engineering** | Java, Python, JavaScript, full-stack development |
| **Architecture** | Cloud-native applications (AWS), microservices, API design |
| **Enterprise Systems** | MYOB transformation, multi-tenant platforms |
| **Methodology** | Agile/Scrum leadership, SAFe, CI/CD pipelines |

---

#### Keith Dimech - COO, Project Director & HSEQ

| Field | Details |
|-------|---------|
| **Position** | COO, Project Director & HSEQ |
| **Total Experience** | 15+ years |
| **Education** | Executive MBA (RMIT, 2025); BSc Honours Geophysics (UniMelb, 2014); BSc Geology (UniMelb, 2011) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **Database Systems** | SQL database design, ESDAT chemical databases, cloud-based tracking systems |
| **Environmental Data** | Contaminated land data, groundwater monitoring, EPA compliance |
| **Carbon Accounting** | Material tracking systems, embodied carbon calculation |
| **Quality Systems** | ISO 9001, ISO 14001, ISO 45001 audit and compliance |

---

### AI/ML Specialists

---

#### Dr. Behnam Sadeghi - Machine Learning Technical Advisor

| Field | Details |
|-------|---------|
| **Position** | Machine Learning Technical Advisor (CSIRO Research Fellow) |
| **Engagement Type** | External Consultant |
| **Total Experience** | 15+ years |
| **Education** | PhD Mining Engineering (Data Analytics), UNSW (2020); MSc Economic Geology (2016); BSc Mining Engineering (2008) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **Machine Learning** | Feature selection, classification, regression, deep learning (CNN, RNN, GAN) |
| **Geospatial AI** | Spatial analysis, mineral prospectivity mapping, remote sensing |
| **Python Development** | Creator of PyImpetus, PyMiner, PySensibo (published packages) |
| **Critical Minerals** | REE exploration, geochemical targeting, mineral systems analysis |

**Published Software:**

| Package | Description | Application |
|---------|-------------|-------------|
| **PyImpetus** | Information-theoretic feature selection | Identifying relevant variables in large datasets |
| **PyMiner** | Mineral prospectivity toolkit | Geospatial AI for exploration targeting |
| **PySensibo** | Time series analysis | Temporal pattern recognition |

**Recognition:**
- **2023 Andrei Borisovich Vistelius Research Award** (IAMG) - highest honor for early-career mathematical geoscientists
- **Fulbright Fellow** (Stanford University, 2022-2023)
- **58+ peer-reviewed publications** (H-index: 24, 2000+ citations)

---

#### Pedro Nogueira Ferreira - AI Software Development Lead

| Field | Details |
|-------|---------|
| **Position** | AI Software Development Lead |
| **Engagement Type** | External Consultant |
| **Total Experience** | 25+ years |
| **Education** | Oxford AI Certification (2024) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **AI/ML** | Enterprise AI strategy, ML model deployment, AI-driven transformation |
| **Cloud Architecture** | Azure, AWS, hybrid cloud solutions |
| **Enterprise Systems** | Backend migrations, legacy modernization |
| **Digital Transformation** | End-to-end enterprise digital strategy |

---

### Geoscience Specialists

---

#### Dr. Qusay Abeed - Senior Geological Consultant

| Field | Details |
|-------|---------|
| **Position** | Senior Geological Consultant / Petroleum Geochemist |
| **Engagement Type** | External Consultant |
| **Total Experience** | 17+ years |
| **Education** | PhD Petroleum Geochemistry & Basin Modelling, RWTH Aachen University (2012, magna cum laude); MSc Petroleum Geology (Baghdad, 2006); BSc Geology (Baghdad, 2002) |
| **Languages** | Arabic (Native), English (Fluent), German (Proficient) |

**Technical Expertise:**

| Category | Competencies |
|----------|--------------|
| **Petroleum Systems** | Basin modelling, source rock characterization, oil-source correlation |
| **CO2 Storage (CCUS)** | Carbon capture, utilisation and storage technologies |
| **Software Development** | C++, Permedia petroleum systems software |
| **Global Experience** | South America, Central America, North Africa, USA, Middle East |

**Current Role:**
- Permedia CO2 Team Agile Scrum Master at Halliburton (2017-Present)
- Lead developer of basin modelling and CO2 storage technologies

---

#### Dr. Fun Meeuws - Geochemistry Data Specialist

| Field | Details |
|-------|---------|
| **Position** | Geochemistry Data Specialist (Part-time) |
| **Employment Type** | Part-time Contractor |
| **Total Experience** | 15+ years |
| **Education** | PhD Geology, University of Adelaide (2013-2019); MSc Geology, Ghent University (2003-2009) |

**Technical Expertise:**
- Apatite fission track thermochronology and facility management
- Thermal history modelling and digitization
- Seismic reflection data interpretation (Kingdom Suite, Geoteric, DUG Insight)
- GPlates paleoreconstructions
- GIS mapping (ArcGIS, QGIS)

**Key Experience:**
- Grant-funded Researcher, University of Adelaide (2019-2024)
- Managed University of Adelaide's apatite fission track facility
- Regional-scale GPlates denudation models for Central Asia

---

#### Dr. Alejandra Bedoya - Data Entry Specialist / PhD Candidate

| Field | Details |
|-------|---------|
| **Position** | Data Entry Specialist / PhD Candidate |
| **Employment Type** | Contractor |
| **Location** | Adelaide, South Australia |
| **Total Experience** | 10+ years |
| **Education** | PhD (in progress), University of Adelaide; MSc, UNAM Mexico (2018); Geological Engineer, Universidad Nacional de Colombia (2015) |

**Technical Expertise:**
- Geochronology and thermochronology
- Tectonics and geological engineering
- Data entry and validation for specialist geoscience datasets

**Prior Experience:**
- Researcher, Servicio Geologico Colombiano (Colombian Geological Survey) (2020-2021)

---

#### Benjamin Dib - Data Clerk (LithoSpace)

| Field | Details |
|-------|---------|
| **Position** | Data Clerk (LithoSpace Platform) |
| **Employment Type** | Part-time Contractor |
| **Total Experience** | 3+ years |
| **Education** | Bachelor of Space Science (BP330), RMIT University (Mar 2022 - Jun 2025) |

**Technical Expertise:**
- Space science and planetary geochemistry
- Moon and Mars mission data processing
- R programming and Excel data tools
- Quality assurance and software testing

**Key Experience:**
- LithoSpace platform data curation
- Milo Space Academy participant (AROSE, 2024)

---

### Software Development Team

---

#### Xinyan Zhang - Frontend Developer

| Field | Details |
|-------|---------|
| **Position** | Frontend Developer |
| **Employment Type** | Full-time |
| **Total Experience** | 3+ years |

**Technical Skills:** React, TypeScript, Next.js, Tailwind CSS, UI/UX implementation, Responsive design

---

#### Lujia Yang - Frontend Developer

| Field | Details |
|-------|---------|
| **Position** | Frontend Developer |
| **Employment Type** | Full-time |
| **Total Experience** | 3+ years |

**Technical Skills:** React, TypeScript, Next.js, Tailwind CSS, UI/UX implementation, Responsive design

**Prior Experience:**
- Software Development Engineer, E Bit (Nov 2021 - Mar 2024)
- Java Developer, Capgemini (Nov 2020 - Oct 2021)

---

#### Tarun Sengar - Backend Developer

| Field | Details |
|-------|---------|
| **Position** | Backend Developer |
| **Employment Type** | Full-time |
| **Total Experience** | 5+ years |

**Technical Skills:** Java, AWS, Backend Systems, API Development, PostgreSQL

---

#### Nirali Dudharejiya - Backend Developer

| Field | Details |
|-------|---------|
| **Position** | Backend Developer |
| **Employment Type** | Full-time |
| **Total Experience** | 5+ years |

**Technical Skills:** Java, Liquibase, Backend Systems, Database Management, PostgreSQL

---

### Data Quality Team

---

#### Cris Ibarra - Data Quality Specialist

| Field | Details |
|-------|---------|
| **Position** | Data Quality Specialist |
| **Employment Type** | Contractor (LATAM) |
| **Total Experience** | 3+ years |
| **Reports To** | Juan Baca |

**Expertise:** Data entry and validation, Quality control processes, Database operations, EarthBank platform

---

#### Perla Luque - Data Quality Specialist

| Field | Details |
|-------|---------|
| **Position** | Data Quality Specialist |
| **Employment Type** | Contractor (LATAM) |
| **Total Experience** | 3+ years |
| **Reports To** | Juan Baca |

**Expertise:** Data entry and validation, Quality control processes, Database operations, EarthBank platform

---

## TECHNICAL SKILLS COVERAGE

| Skill | Staff Count | Proficiency | Key Staff |
|-------|-------------|-------------|-----------|
| Cloud Platform (AWS) | 4+ | Expert | Wayne Noble, Tarun Sengar, Pedro Ferreira |
| Python Development | 3+ | Expert | Dr. Sadeghi, Pedro Ferreira, Dr. Kohlmann |
| Database (PostgreSQL) | 4+ | Expert | Moritz Theile, Wayne Noble, Keith Dimech |
| REST API Development | 4+ | Expert | Wayne Noble, Tarun Sengar, Nirali Dudharejiya |
| Frontend (React/TypeScript) | 3+ | Expert | Wayne Noble, Xinyan Zhang, Lujia Yang |
| Machine Learning | 2+ | Expert | Dr. Sadeghi, Pedro Ferreira |
| Geoscience Domain | 4+ | Expert | Dr. Kohlmann, Dr. Sadeghi, Dr. Abeed, Keith Dimech |

---

## ACADEMIC PUBLICATIONS (KEY)

| Year | Title | Authors | Journal |
|------|-------|---------|---------|
| 2023 | A geospatial platform for low-temperature thermochronology Big Data | Boone, **Kohlmann**, **Noble**, **Theile**, et al. | Scientific Reports (Nature) |
| 2022 | AusGeochem: An open platform for geochemical data | Nixon et al., **Theile**, Gréau, **Noble**, et al. | Geostandards and Geoanalytical Research |
| 2025 | Deep-Time Digital Twins: LithoPlates + EarthBank | **Kohlmann**, **Noble**, Qin, Higton, et al. | EGU General Assembly 2025 |

---

## DECLARATION

I hereby certify that all information provided regarding professional staff experience is true, accurate, and complete to the best of my knowledge.

| Field | Entry |
|-------|-------|
| **Name** | Keith Dimech |
| **Title** | Chief Operating Officer |
| **Signature** | _________________________ |
| **Date** | _________________________ |
| **Company Seal** | [AFFIX HERE] |

**Authorized Signatory:** Dr. Fabian Kohlmann (Managing Director)

---

*Form 9.4 - Professional Staff Experience*
*Lithodat Pty Ltd - ABN 63 627 008 904*
*Prepared for GDAC-SA RFQ Pre-Qualification*

---

## OFFLINE VERSION CHANGES

**This section documents differences between the live tender review system and this offline submission-ready version.**

### Additions (Content from ChatGPT Research)

| Section | Addition |
|---------|----------|
| **Staff Table** | Expanded to 13 professional/technical staff with specializations |
| **Dr. Behnam Sadeghi** | Added full profile: CSIRO Research Fellow, 2023 Vistelius Award, Fulbright Fellow, PyImpetus/PyMiner creator |
| **Pedro Nogueira Ferreira** | Added AI Lead profile: Oxford AI Certification, 25+ years enterprise experience |
| **Dr. Qusay Abeed** | Added Senior Geological Consultant: Arabic speaker, RWTH Aachen PhD, Halliburton Permedia team |
| **Vinko Novak** | Added Head of Data Security role |
| **Development Team** | Added 4 developers: Xinyan Zhang, Lujia Yang, Tarun Sengar, Nirali Dudharejiya |
| **Data Quality Team** | Added Cris Ibarra, Perla Luque, Dr. Alejandra Bedoya |
| **Dr. Fun Meeuws** | Added Geochemistry Data Specialist: fission track, thermal history, GPlates |
| **Benjamin Dib** | Added Data Clerk (LithoSpace): space science, RMIT student |
| **Technical Skills Coverage** | Added matrix showing skill coverage across team |
| **Academic Publications** | Added 3 key publications (Scientific Reports, Geostandards, EGU 2025) |

### Modifications

| Field | Live Version | Offline Version |
|-------|--------------|-----------------|
| **Staff count** | Basic list | Expanded to 13 with detailed technical profiles |
| **AI/ML capabilities** | Not detailed | Added 2 specialist profiles with published software packages |
| **Geoscience expertise** | Generic | Added specific domain expertise (thermochronology, isotopes, petroleum) |

### Removals

None - all live content retained and significantly expanded.

### Notes

- Benjamin Dib's role "Data Clerk (LithoSpace)" retained as this is his actual job title, not a similar project reference
- LithoSpace platform work is ongoing internal R&D, separate from the 3 similar projects (EarthBank, Isotopes.au, NRCan)
