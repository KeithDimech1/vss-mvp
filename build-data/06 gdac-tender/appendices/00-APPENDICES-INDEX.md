# GDAC-SA TENDER - APPENDICES INDEX
## Lithodat Pty Ltd - Pre-Qualification Response

**RFQ Reference:** 251140007625
**Date Created:** December 2025
**Total Appendices:** 13 (A-M)

---

## Appendix List

| Appendix | Document Title | File Type | Size | Reference Section |
|----------|---------------|-----------|------|-------------------|
| **A** | ASIC Company Extract (02-Dec-2025) | PDF | 526 KB | Form 9.1 - Company Information |
| **B** | Financial Statements FY2025 (Year ended 30 June 2025) | PDF | 207 KB | Form 9.6 - Financial Capacity |
| **C** | Financial Statements FY2024 (Year ended 30 June 2024) | PDF | 261 KB | Form 9.6 - Financial Capacity |
| **D** | Financial Statements FY2023 (Year ended 30 June 2023) | PDF | 247 KB | Form 9.6 - Financial Capacity |
| **E** | Quality Management System (QMS) Policy | Markdown | 6.0 KB | Form 9.2 - Quality Assurance |
| **F** | Health, Safety & Environment (HSE) Policy | Markdown | 6.2 KB | Form 9.2 - HSE Standards |
| **G** | EarthBank Project Summary & Client Reference | Markdown | 6.0 KB | Form 9.5 - Similar Projects |
| **H** | Isotopes.au Project Summary & Client Reference | Markdown | 6.0 KB | Form 9.5 - Similar Projects |
| **I** | NRCan Project Summary & Client Reference | Markdown | 6.0 KB | Form 9.5 - Similar Projects |
| **J** | Curriculum Vitae (Combined) | Markdown | 42 KB | Form 9.3 - Administrative Staff |
| **K** | Publications Summary | Markdown | 2.4 KB | Form 9.2 - Innovation Excellence |
| **L** | Elsevier Book Reference - "Fractals and Multifractals in the Geosciences" | Markdown | 1.8 KB | Form 9.4 - Professional Staff |
| **M** | Project Organizational Chart | Markdown | 5.9 KB | Form 9.1 - Company Structure |

---

## File Naming Convention

All files follow the naming pattern: `[Letter]-[Descriptive-Name].[ext]`

Example: `A-ASIC-Company-Extract-02-Dec-2025.pdf`

---

## Document Categories

### Corporate & Financial (A-D)
- **4 documents** - ASIC registration and 3 years of financial statements
- **Total Size:** ~1.2 MB
- **Format:** PDF

### Policies (E-F)
- **2 documents** - QMS and HSE policies
- **Total Size:** ~12 KB
- **Format:** Markdown

### Projects (G-I)
- **3 documents** - EarthBank, Isotopes.au, NRCan project summaries
- **Total Size:** ~18 KB
- **Format:** Markdown

### Personnel & Publications (J-M)
- **4 documents** - CVs, publications, book reference, org chart
- **Total Size:** ~52 KB
- **Format:** Markdown

---

## Contents Summary

### Appendix A - ASIC Company Extract
**Source:** ASIC (Australian Securities and Investments Commission)
**Date:** 02-Dec-2025
**Document ID:** 7ECW02160
**Contains:**
- Company registration details
- Shareholder structure (5 shareholders, 1,000 shares total)
- Director information
- Registered address

### Appendix B, C, D - Financial Statements
**Source:** Scott Partners Pty Ltd (Accountants)
**Years:** FY2023, FY2024, FY2025
**Contains:**
- Balance Sheet
- Profit & Loss Statement
- Notes to Financial Statements
- Directors' Declaration
- Independent Auditor's Report

### Appendix E - QMS Policy
**Document Type:** Quality Management System Policy
**Status:** ISO 9001-aligned
**Contains:**
- Quality Policy Statement (CEO-signed)
- Peer Review Standard Operating Procedures
- Competent Person Register
- Data Validation Checklists

### Appendix F - HSE Policy
**Document Type:** Health, Safety & Environment Policy
**Status:** ISO 45001-aligned
**Contains:**
- HSE Policy Document (CEO-approved)
- Risk Assessment Framework
- Emergency Response Procedures
- Training and Competency Management

### Appendix G, H, I - Project Summaries
**Projects:**
- **G:** EarthBank - AUD $2.9M, 2020-Present
- **H:** Isotopes.au - AUD $400K, 2024-2028
- **I:** NRCan Canada - AUD $400K, 2021-2023 (Delivered)

**Contains:**
- Project descriptions
- Contract values
- Client information
- Deliverables and outcomes

### Appendix J - Curriculum Vitae (Combined)
**Personnel Included:**
1. Dr. Fabian Kohlmann - Managing Director & CEO
2. Dr. Wayne Noble - Chief Information Officer
3. Keith Dimech - Chief Operating Officer
4. Dr. Behnam Sadeghi - ML Technical Advisor
5. Dr. Mahdi AbuAli - GDAC Saudi Lead
6. Dr. Qusay Abeed - GDAC Technical Director

**Total Pages:** ~40 pages combined

### Appendix K - Publications Summary
**Content:**
- Chemical Geology (2025) publication details
- Dr. Behnam Sadeghi publication list (58+ articles)
- Awards and recognition
- Python packages developed (3)
- Editorial positions

### Appendix L - Elsevier Book Reference
**Book:** "Fractals and Multifractals in the Geosciences"
**Author:** Dr. Behnam Sadeghi
**Publisher:** Elsevier (2024)
**ISBN:** 978-0-323-90897-9
**URL:** https://www.elsevier.com/books/fractals-and-multifractals-in-the-geosciences/sadeghi/978-0-323-90897-9

### Appendix M - Organizational Chart
**Content:**
- Corporate shareholding structure
- Management hierarchy
- GDAC-SA project team structure
- Department breakdown (19 total employees)
- Location information
- Saudization status

---

## Usage Instructions

### For Etimad Portal Submission:

1. **PDF Documents (A-D):**
   - Upload directly to Etimad portal
   - Already in required PDF format
   - No conversion needed

2. **Markdown Documents (E-M):**
   - Convert to PDF before uploading
   - Use Pandoc or similar tool
   - Or copy content into Word/PDF template

3. **File Size:**
   - Total size: ~1.3 MB
   - Well within Etimad portal limits
   - All files can be uploaded together

---

## Conversion Commands (if needed)

To convert Markdown files to PDF:

```bash
# Using Pandoc
pandoc E-Quality-Management-System-QMS-Policy.md -o E-QMS-Policy.pdf

# Or batch convert all markdown files
for file in *.md; do
  pandoc "$file" -o "${file%.md}.pdf"
done
```

---

## Verification Checklist

- ✅ All 13 appendices present (A-M)
- ✅ File naming consistent
- ✅ Financial statements cover 3 years (FY2023-2025)
- ✅ ASIC document is most recent (02-Dec-2025)
- ✅ All policy documents included
- ✅ Project references complete
- ✅ CV information comprehensive
- ✅ Publications documented
- ✅ Organizational structure clear

---

## Notes

- **Language:** All documents are in English. Arabic translations should be prepared where required by Saudi procurement regulations.
- **Updates:** If any appendix needs updating, maintain the file naming convention and update this index.
- **Backups:** Original source files retained in `learning/` folders.

---

**Prepared By:** Keith Dimech, Chief Operating Officer
**Company:** Lithodat Pty Ltd
**For:** GDAC-SA Pre-Qualification Response (RFQ 251140007625)
**Date:** December 2025

---

*End of Appendices Index*
